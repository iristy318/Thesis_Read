...
//    std::cout<<faces.size()<<std::endl;
    if (faces.size() > 0) {
        faceVec.insert(faceVec.begin(), 1);
        outFrame = findEyes(frame_gray, faces[0]);
        std::cout<<"face shows" << faceVec[0] <<std::endl;
    }
    else{
        faceVec.insert(faceVec.begin(), 0);
        outFrame = frame_gray;
//        std::cout<<outFrame.size()<<std::endl;
    ...
    std::cout<<"acc " << acc << " out of " << 10 <<std::endl;
    if(acc > 0.8*float(faceVec.size())){
        isFaceNew = true;
    }
    else if (acc < 0.6*float(faceVec.size()))
    {
        isFaceNew = false;
    }
    std::cout << "face is " << isFaceNew << std::endl;
...
//    printer.open("/dev/tty.SLAB_USBtoUART");
////    printer.setReverse(false);
////    printImg.load("screenshot_540.png");
...
//    ofLoadImage(pixels, "printscreen.png");
    ...
    isprintResultCalled = true;
}

void ofApp::resetCounterVec(){
